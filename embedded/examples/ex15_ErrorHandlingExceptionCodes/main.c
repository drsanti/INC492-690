...
void FunctionCode06() {
    ...
    if(freq < 100 || freq > 1e3) {         // 100Hz - 1kHz
        ...
        buffer_r[0] = buffer[0];            // Slave Address
        buffer_r[1] = 0x80|buffer[1];       // 1000 0000 (0x80) -- Function Code
        buffer_r[2] = 0x03;                 // Error Code (0x03: Illegal Data Value)
        uint16_t crc = CalculateModbusCRC(buffer_r, 3);
        buffer_r[3] = (uint8_t)(crc>>8);    // CRC Low
        buffer_r[4] = (uint8_t)(crc&0xFF);  // CRC High
        Uart1_WriteBytes((char *)buffer_r, 5);
...
void Interpreter(void) {
    ...
    if( counter == 8 ) {                                // FC 0x01-0x06
        ...
        if(crcM != crcC) {
            ...
            buffer_r[0] = buffer[0];
            buffer_r[1] = 0x80|buffer[1];       // 1xxx xxxx => 1000 0000 (0x80)
            buffer_r[2] = 0x07;                 // Error Code (0x07: Negative Acknowledge)
            uint16_t crc = CalculateModbusCRC(buffer_r, 3);
            buffer_r[3] = (uint8_t)(crc>>8);    // CRC Low
            buffer_r[4] = (uint8_t)(crc&0xFF);  // CRC High
            Uart1_WriteBytes((char *)buffer_r, 5);
            ...
            buffer_r[0] = buffer[0];
            buffer_r[1] = 0x80|buffer[1];       // 1xxx xxxx => 1000 0000 (0x80)
            buffer_r[2] = 0x01;                 // Error Code (0x01: Illegal Function)
            uint16_t crc = CalculateModbusCRC(buffer_r, 3);
            buffer_r[3] = (uint8_t)(crc>>8);    // CRC Low
            buffer_r[4] = (uint8_t)(crc&0xFF);  // CRC High
            Uart1_WriteBytes((char *)buffer_r, 5);
...
int main(void) {
    System_Init();
    Uart2_PrintfAsync("Error Handling and Exception Codes\r\n");  // UART2
